...
DomItem* DomModel::getRoot() { return rootItem; }
...
QVariant DomModel::headerData(int section, Qt::Orientation orientation,
                              int role) const {
  if (role == Qt::TextAlignmentRole) return QVariant();
...
QModelIndex DomModel::addItem(const QModelIndex& parent, int row,
                              ItemState* state) {
  if (parent.isValid()) {
    // QModelIndex index = this->index(parent.row(), 0, parent.parent());
    // //原始,有导致app崩溃的情况发生
    QModelIndex index =
        parent;  //粘贴时使用，否则导致新建子级项无法撤销（索引无效）
    DomItem* item = itemForIndex(index);
...
QModelIndex DomModel::addMoveItem(const QModelIndex& parent, int row,
                                  ItemState* state) {
  if (parent.isValid()) {
...
QModelIndex DomModel::pasteItem(const QModelIndex& parent, int row,
                                ItemState* state, DomItem* copy_item) {
  Q_UNUSED(state);
  ...
  if (parent.isValid()) {
    ...
    if (item->getType() == "array") {
      child->setData("Item " + QString::number(child_count + 1),
                     copy_item->getType(), copy_item->getValue());
    ...
    if (item->getType() != "array") {
      for (int i = 0; i < count; i++) {
        ...
        tab->treeView->setCurrentIndex(
            tab->getModel()->index(i, column, index));

        item1 = item->child(i);

        if (i + 1 < count) item2 = item->child(i + 1);
...
//将items中的数据，编译成plain text并存储在QMimeData对象中
{
  ...
  if (indexes.at(0).parent().isValid())
    childCount =
        tab->getModel()->itemForIndex(indexes.at(0).parent())->childCount();

  return mimeData;
}

bool DomModel::dropMimeData(const QMimeData* data, Qt::DropAction action,
