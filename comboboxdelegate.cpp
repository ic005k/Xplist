...
ComboBoxDelegate::ComboBoxDelegate(QObject* parent) { Q_UNUSED(parent); }
...
void ComboBoxDelegate::setEditorData(QWidget* editor,
  ...
  /*if (red < 55)  // mac = 50
  {
    comboBox->setStyleSheet(
        ...
        "gray;background:rgba(50,50,50,255);selection-color: "
        "#277BFF;selection-background-color:#FFFFFF;}");
  } else {
    comboBox->setStyleSheet(
        "QComboBox {border:1px solid "
...
void ComboBoxDelegate::setModelData(QWidget* editor, QAbstractItemModel* model,
  ...
  if (oldValue != val) {
    ...
    if (item->getName() != "plist") {
      ...
      if (val == "integer") {
        if (CurrentValue.trimmed() == "false") CurrentValue = "0";

        if (CurrentValue.trimmed() == "true") CurrentValue = "1";
      ...
      if (val == "date") {
        NewValue = QDate::currentDate().toString("yyyy-MM-ddT") +
